...
namespace Kindle.IntegrationUtilities.RMB
{
    internal class AdditionalInfoParser
    {
        ...
        private void DetectFormat(ref Information information)
        {
            ...
            else if (_reader.ToString().Trim().Length <= 65)
            {
                ReadInfoToOwner(ref information);
        ...
        private void ReadInfoToOwner(ref Information information)
        {
            var value = _reader.Peek(65); //_reader.Read();
            if (value.Length <= 0)
            {
            ...
            if (value.Contains("\r\n"))
            {
                ...
                AdditionalInfoToOwner(ref information);
            }
            else
            {
                _reader.Skip(42);
            ...
            if (value.Length < 1)
            {
                throw new InvalidDataException("The statement line data ended unexpectedly. Expected \"Information To Owner\" with a length of at least 1 character.");
            }
            ...
            information.InfoToOwner = value;
        }

        private void AdditionalInfoToOwner(ref Information information)
        {
            var value = _reader.Read(65);
            if (value.Length <= 0)
            {
                return;     // End of buffer
            }
            ...
            if (!value.StartsWith("\r\n"))
            {
                throw new InvalidDataException($"Unexpected data found. Expected \"Additional Information To Owner\". Actual {value}");
            }

            value = value.Substring(2);
            information.AdditionalInfoToOwner = value;
        }
    }
}
